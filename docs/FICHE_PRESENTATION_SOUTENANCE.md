...
*   **Plan B (Fallback PDF ou Choix Utilisateur) :** Si le texte natif est manquant (ou si l'utilisateur coche l'option de forçage manuel), le script utilise l'API officielle v2 de ClinicalTrials pour récupérer et télécharger directement le PDF original de l'essai via leur CDN. Le fichier est ensuite envoyé **directement dans notre bucket Cloud Supabase** (dossier `clinical_pdfs`) pour le sauvegarder, puis envoyé au serveur GPU.
    > **💡 Point d'Ingénierie Fort (Scraping CDN vs Playwright) :** Au début du projet, nous utilisions un navigateur fantôme (`Playwright`) pour simuler un clic humain sur le site. Nous avons remplacé cela par une interrogation directe du CDN via l'API v2. Résultat : une extraction instantanée, aucune dépendance lourde de navigateur web, et une robustesse à 100% pour un déploiement Cloud (sur Render).
